...
RealApiMonitor.monitor(context)
    ...
    .timeout(30)                  // 超过 timeout 时间：自动停止监控
    .start();                     // 异步启动，立即返回
...
| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| `monitor(context)` | BrowserContext | MonitorBuilder | 创建监控构建器 |
...
| `setTargetHost(host)` | String | void | 设置目标 Host |
