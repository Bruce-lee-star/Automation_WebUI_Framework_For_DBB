...
| P1 | T1-1~T1-9（门禁 7 件套）| 🔶 部分 | T1-6 ArchUnit ✅（7 规则：page↔route 双向解耦 / common→web·api 越层 / 顶层切片无环）；T1-2 Checkstyle ✅（verify 门禁，作用域限定重构包，0 违规）；T1-1 JaCoCo 🔶（prepare-agent+report 已接线，check 门禁因框架单测为行为护盾、覆盖率约 0% 暂未启用，待 T1-9）；T1-3 SpotBugs / T1-4 OWASP / T1-5 Enforcer 🔶（已接线 3 条安全规则：maven/java 版本锁 + banDuplicatePomDependencyVersions，7 模块全通过；requireUpperBoundDeps 因 7 处版本收敛冲突暂缓，留待依赖收敛专项）/ T1-7 Mockito·AssertJ（测试 classpath 可用，建议显式声明）/ T1-8 CI / T1-9 覆盖率补测 待办 |
