...
> - **🔗 SSO 感知并发（按身份分区互斥）核心已落地（2026-09-07）**：框架应按 `(环境, 用户名)` 等身份维度决定并发/串行。核心抽象（`ConcurrencyPartitionKey` + `ConcurrencyGate` + `ConcurrencyKeyResolver` + `LoginIdentityKeyResolver` + `TagOverrideKeyResolver` + `ConcurrencyIdentity` + `ConcurrencyScope`）已落地于 `web` 模块 `framework.web.concurrent` 包，`FrameworkConfig` 三项配置到位，`ConcurrencyGateTest` 9 例全绿；闸门默认关闭（`CONCURRENCY_PARTITION_ENABLED=false`）→ 全护盾零回归。与 `LoginGuard` 单飞互补、runner 无关且串行 no-op。**生命周期 hook 接入（scenario 登录后 `enter`、结束 `release`）随 JUnit 5 迁移一并实施**。详见 `CONCURRENT_CONTEXT_EXECUTOR_DESIGN.md` 附录 A。
> - **🔗 企业级并发韧性 BrowserCrashGuard 已落地（2026-09-07）**：解决共享 Browser 单点（风险 R7）。`web.lifecycle` 新增 `BrowserCrashGuard`（崩溃检测 `isCrash` + 进程级单飞恢复 `recover` + 配置开关，默认开启 `CONCURRENCY_BROWSER_CRASH_GUARD_ENABLED=true`）；`PlaywrightManager` 新增包级私有 `rebuildSharedBrowserIfDisconnected()`（共享模式 `SHARED_BROWSER_LOCK` 内重建、幂等；非共享模式放行由 `getBrowser()` 自动重建）；`ConcurrentContextExecutor.executeOne` 抽取 `runOnce`，崩溃型失败经 `recover()` 后重跑一次（有界 `MAX_REPLAY=1`）。`BrowserCrashGuardTest` 9 例全绿，全护盾 428 例零回归。身份亲和由"原 worker 线程重跑"自然满足。详见 `CONCURRENT_CONTEXT_EXECUTOR_DESIGN.md` 附录 A.12 / 第九节 9.6。
