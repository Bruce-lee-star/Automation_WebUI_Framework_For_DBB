...
| C-6 | **P1** | 配置解析路径分裂（`ConfigSource` vs `FrameworkFlags`），无配置键统一注册表 **已修复**：core `ConfigKeys` 注册表已加重复键 fail-fast 静态校验（新增 `ConfigKeysRegistryTest` 3/3），**2026-09-20 完成全量收敛**：Web / API / Monitor 三侧的键定义已全部迁移至 core `ConfigKeys`（184 键，唯一事实来源），三侧（`WebFrameworkConfig` / `ApiFrameworkConfig` / `MonitorConfig`）退化为**读取门面**；默认值不漂移由 **golden 快照**（`ConfigKeysGoldenTest`，以迁移前基线逐键逐字比对）守卫，门面不得重新持有本地定义由 `ConfigUnificationTest#facadesMustDelegateToRegistry` 守卫；**键唯一性**（`configKeysAreGloballyUnique`，Web×API）保留。原「镜像默认值一致性」校验随镜像模型一并退役（镜像已不存在，该风险在结构上被消除） | `ConfigKey.java:17`、`FrameworkFlags.java:29-71` | 新配置无处可查，键名无静态校验 |
