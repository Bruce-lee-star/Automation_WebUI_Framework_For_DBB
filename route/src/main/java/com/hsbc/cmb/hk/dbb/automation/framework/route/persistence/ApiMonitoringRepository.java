 ...
 *   <li>自动建表：首次连接成功后执行 schema 迁移（DDL 已外置为 db/migration 资源，旧库自动基线）</li>
...
public final class ApiMonitoringRepository {
     ...
     */
    /** 支持的持久化方言（DDL 已按各库语义对齐：MySQL/PostgreSQL/H2/Oracle/SQLServer）。
     *  其余未知方言前置 fail-fast 拒绝，避免错配建表。 */
    private static final Set<String> SUPPORTED_DIALECTS =
            Set.of("MYSQL", "POSTGRESQL", "H2", "ORACLE", "SQLSERVER");
    ...
    public static synchronized void init(String dbUrl, String dbUser, String dbPassword,
        ...
        // 修复 ROUTE-P1-N1：方言与 DDL 强绑定；未知/不支持的方言若静默回落 MySQL DDL，
        // 会在目标库建表阶段才以晦涩错误失败（且 Oracle 不支持 AUTO_INCREMENT）。
        // 在此前置 fail-fast：明确拒绝并退回（数据不落库），而非用错配 DDL 建表。
        // 注：Oracle/SQLServer 现已纳入 SUPPORTED_DIALECTS（各自 V1 迁移 SQL 已对齐），
        // 仅真正未知的类型（不在 SUPPORTED_DIALECTS 内）会被拒绝。
        if (!SUPPORTED_DIALECTS.contains(dialect)) {
            LOGGER.error("[ApiMonitoringRepository] Unsupported DB dialect '{}' (supported: {}). "
                    + "API monitor data will NOT be persisted. Refusing to fall back to MySQL DDL "
                    + "to avoid schema mismatch.", dialect, SUPPORTED_DIALECTS);
            initFailed = true;
            return;
        }
        ...
        try {
            ...
            // 通过 Flyway 管理 route_monitor_record schema（ROUTE-P1-N2：DDL 已移出 Java，
            // 外置为 db/migration/<dialect>/V1__create_route_monitor_record.sql）
            migrateSchema(dataSource, dialect);
    ...
    /**
     * 通过 Flyway 管理 {@code route_monitor_record} schema（ROUTE-P1-N2）。
     * <p>DDL 已完全移出 Java，落地为 classpath 资源
     * {@code db/migration/<dialect>/V1__create_route_monitor_record.sql}；按解析出的方言选择对应迁移目录，
     * 延续「测试层加哪个驱动依赖就适配哪种库」的零硬编码设计。
     * <ul>
     *   <li>新库（空 schema）：Flyway 执行 V1 建表并写入 {@code flyway_schema_history}，后续迁移有序演进；</li>
     *   <li>旧库（已有表、无 flyway 历史）：{@code baselineOnMigrate=true} 自动基线到 V1（跳过执行），
     *       既不动已有表结构也不丢数据，满足「旧库可迁移」。</li>
     * </ul>
     * SQL 内保留 {@code IF NOT EXISTS}，即便基线分支未触发也幂等安全。
     *
     * @param ds      已验证的 Hikari 连接池（Flyway 复用其连接）
     * @param dialect 解析出的方言（MYSQL / POSTGRESQL / H2）
     * @throws Exception 迁移失败（交由 {@link #init} 外层捕获并静默降级，不抛到调用方）
     */
    private static void migrateSchema(HikariDataSource ds, String dialect) throws Exception {
        String location = "classpath:db/migration/" + dialectToMigrationFolder(dialect);
        Flyway.configure()
                .dataSource(ds)
                .locations(location)
                ...
                .load()
                .migrate();
        LOGGER.info("[ApiMonitoringRepository] Schema migrated successfully (location={})", location);
    }

    /** 方言 → Flyway 迁移目录（与 {@code db/migration} 下文件夹一一对应；包级私有供白盒测试）。 */
    static String dialectToMigrationFolder(String dialect) {
