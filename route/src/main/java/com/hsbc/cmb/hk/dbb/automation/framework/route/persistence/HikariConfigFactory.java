 ...
 *
 * <p>置于 route（其唯一真实消费方 {@code ApiMonitoringRepository}），使 core 回归零存储依赖
 * （D6-1：core 不再依赖 HikariCP）。原由 web 的 {@code framework.web.utils.HikariConfigFactory}
 * 下沉到 core，现进一步归位到 route，依赖方向保持单向（route → core）。行为与原内联配置保持一致
 * （未显式设置的字段沿用 Hikari 默认值）。
 *
