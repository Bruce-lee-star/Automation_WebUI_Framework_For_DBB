...
public class FullRouteFrameworkTest {
    ...
    @Before
    public void setUp() {
        // 每个 @Test 方法运行前创建独立的 BrowserContext / Page。
        // ApiCaptureContext 为静态单例，由 RouteDsl.on(page) 内部绑定当前 page；
        // 这里只需保证 page / context 实例字段非空，供各测试方法直接引用。
        context = browser.newContext();
        page = context.newPage();

        // ⭐ 启动 capture 目录的采集管道，统一接管所有类型 API 的存储。
        //    MONITOR/DELAY 的 resume 放行请求、MODIFY 的 route.fetch 子请求由 CDP 旁路实时捕获；
        ...
        //    （modify/mock 类测试会真实修改 /api/users，若不重置会污染后续断言）。
        //    服务不可达时忽略（不影响本地离线运行）。
        try {
            java.net.HttpURLConnection resetConn =
                    (java.net.HttpURLConnection) new java.net.URL(BASE_URL + "/api/reset").openConnection();
            resetConn.setRequestMethod("POST");
            resetConn.setConnectTimeout(2000);
            resetConn.setReadTimeout(2000);
            resetConn.getResponseCode();
            resetConn.disconnect();
        } catch (Exception ignored) {
            // demo service 未启动时不阻断测试
        }
    }

    @After
    public void tearDown() {
        // ⭐ 停止 capture 采集引擎，确保其异步合并线程（EventMerger）完全退出，
        //    避免跨测试残留写入共享 ApiCaptureContext 导致数据污染（批量运行时尤为关键）。
        try { ApiCapture.stop(); } catch (Exception ignored) {}
        // ⭐⭐ 兜底全局清理：若某测试断言失败抛异常跳到 tearDown，其方法内局部 dsl 无法被访问，
        //    导致该测试注册的路由 / MonitorSession 定时器 / CONTEXT_RULES / PAGE_RULES 等静态状态
        //    残留到下个 case（线程/路由污染）。此处无条件清空所有路由状态（stop 定时器 + 清空静态 Map），
    ...
    public void b14_mock_batch_replace_nested() {
        RouteDsl dsl = RouteDsl.on(page)
                ...
                .replaceField("$.orders[*].price", "0.01")
                .replaceField("$.orders[*].product", "FREE_ITEM")
                .allowAllRequests()
                .done();
        dsl.start();
    ...
    public void c17_modify_request_headers() {
        ...
        // ModifyHandler 将修改详情序列化为 JSON 存储在 CapturedApiCall.modifyDetail 中
        // （Q1：MODIFY 只改请求、不改响应，故修改标记不在 responseBody，而在 modifyDetail）
        assertEquals("X-Request-ID should be set", "test-123",
                call.modifyDetailJson("$.headersSet.X-Request-ID"));
        assertEquals("X-Forwarded-For should be set", "127.0.0.1",
                call.modifyDetailJson("$.headersSet.X-Forwarded-For"));
        assertNotNull("HeadersRemoved info should be present", call.modifyDetailJson("$.headersRemoved"));
    ...
    public void c18_modify_request_body_fields() {
        ...
        // ModifyHandler 存储的 JSON（modifyDetail）包含 bodyFieldsModified/bodyFieldsAdded/bodyFieldsRemoved
        // key 是 DSL 传入的 JsonPath 表达式（如 "$.role"），需用 bracket notation
        assertEquals("role should be modified", "MODIFIED_ADMIN",
                call.modifyDetailJson("$.bodyFieldsModified['$.role']"));
        assertEquals("extraField should be added", "added_value",
                call.modifyDetailJson("$.bodyFieldsAdded['$.extraField']"));

        Object removed = call.modifyDetailJson("$.bodyFieldsRemoved");
    ...
    public void c19_modify_http_method() {
        ...
        CapturedApiCall call = ctx.getLastApiCall("/api/users/1");
        assertNotNull(call);
        assertEquals("HTTP method should be PUT", "PUT", call.modifyDetailJson("$.modifiedMethod"));
    ...
    public void c20_modify_complex_jsonpath() {
        ...
        // 验证修改后的 body 包含预期字段（modifyDetail 中记录实际发出的请求体）
        String modifiedBody = (String) call.modifyDetailJson("$.modifiedBody");
    ...
    @Test
    public void c21_monitor_baseline_with_modify_delay_overlay() throws Exception {
        // 设计对齐：MONITOR 是非可覆盖基线，先注册；进入功能后再叠加 MODIFY + DELAY，
        // 三者同时生效 —— MODIFY 只改请求、不改响应，monitor 对真实（透传）响应断言。
        // 1) 先注册 monitor 基线（监控 /api/users 的真实响应健康度）
        //    监控对真实响应断言：期望 HTTP 200（验证 MOCK/modify 未篡改响应状态，
        //    且真实响应原样透传）。用 status 断言而非具体业务字段，避免受共享服务数据污染影响。
        RouteDsl baseline = RouteDsl.on(page)
                .api("/api/users")
                .monitor()
                .expectStatus(200)
                .allowAllRequests()
        ...
        // 2) 进入功能后叠加 MODIFY（改请求头）
        RouteDsl overlayModify = RouteDsl.on(page)
                .api("/api/users")
                .modifyRequest()
                .setRequestHeader("X-Request-ID", "req-overlay-001")
                .allowAllRequests()
                .done();
        overlayModify.start();

        // 3) 再叠加 DELAY（延迟 1 秒，最小单位为秒）
        RouteDsl overlayDelay = RouteDsl.on(page)
                .api("/api/users")
                .delay(1)
                .allowAllRequests()
                .done();
        ...
        ApiCaptureContext ctx = ApiCaptureContext.getCurrent();
        CapturedApiCall call = ctx.getLastApiCall("/api/users");
        assertNotNull("Overlay call should be captured", call);

        // DELAY 生效：请求总耗时 ≥ 1000ms
        assertTrue("DELAY should take effect (elapsed >= 1000ms), actually " + elapsed,
                elapsed >= 1000);

        // MODIFY 生效：请求头被修改（记录在 modifyDetail）
        assertEquals("Request header X-Request-ID should be set by overlay",
                "req-overlay-001", call.modifyDetailJson("$.headersSet.X-Request-ID"));

        // Q1：不改响应 —— 真实响应透传，monitor 对真实响应断言通过（200），响应体应为原始 JSON 数组
        String respBody = call.responseBody();
        assertNotNull("Real response body should be passed through untouched", respBody);
        assertTrue("Real response should be an array of users (modify did NOT replace/alter the response)",
                respBody.trim().startsWith("["));

        // monitor 基线未被 modify/delay 覆盖：断言通过即不抛异常（已隐式验证）
        baseline.clear();
        overlayModify.clear();
        overlayDelay.clear();
    }
    ...
    public void e25_complex_business_scenario() throws Exception {
        RouteDsl dsl = RouteDsl.on(page)
                ...
                .mockStatus(200)
                .replaceField("$.permissions[*]", "ALL_ACCESS")
                .allowAllRequests()
                .done()
                .api("/api/slow/endpoint")
                .delay(1)
    ...
    public void m40_modifyHandler_replaceBatchByWildcard() {
        ...
        Map<String, Object> reps1 = new HashMap<>();
        ...
        String json2 = "{\"users\":[{\"orders\":[{\"price\":100},{\"price\":200}]}," +
                "{\"orders\":[{\"price\":300}]}]}";
        Map<String, Object> reps2 = new HashMap<>();
        ...
        Map<String, Object> reps3 = new HashMap<>();
        ...
        Map<String, Object> reps5 = new HashMap<>();
        reps5.put("$.x", "y");
        String result5 = com.hsbc.cmb.hk.dbb.automation.framework.web.route.handler.ModifyHandler
    ...
    public void m43_routeRule_equals_hashCode() {
        ...
        // type 不参与相等性（能力位 MERGE 模型：同 pattern 规则视为同一逻辑规则，
        // 由 mergeFrom() 叠加能力位，type 仅作终结标记）。相同 urlPattern 不同 type 仍相等。
        assertEquals("Same urlPattern, different type should STILL be equal (type not in equals)", r1, r4);
        assertEquals("Equal objects -> same hashCode", r1.hashCode(), r4.hashCode());
        ...
        map.put(r4, "third");  // 新模型：type 不参与 equals，r4 与 r1 同 key → 覆盖
        assertEquals("r4 should overwrite r1 (same key, type not in equals)", "third", map.get(r1));
        assertEquals("Should have 1 entry total (type not part of key)", 1, map.size());
    }
    ...
    public void n49_context_delay_page_monitor() {
        ...
        try {
            ...
            // ⭐ 用户诉求：DELAY 只是延迟放行，不覆盖 MONITOR。
            //   请求被延迟 500ms 后放行，MONITOR 对真实响应尽力监控。
            //   注意：纯 MONITOR 在跨层 DELAY 异步线程读 req.response() 存在 Playwright 固有竞态
            //   （偶发 "Object doesn't exist"），MonitorHandler 已加失效保护（读不到则安全跳过，不 crash 路由）。
            //   故此处不强断言「一定监控到数据」，只验证 DELAY 生效 + 无断言失败 + 不崩溃。
            assertTrue("Elapsed should include context delay (500ms), actual: " + elapsed + "ms",
                    elapsed >= 450);

            // MONITOR 不被 DELAY 覆盖：无断言失败（若成功读到真实响应，则断言健康）
            ApiCaptureContext ctx = ApiCaptureContext.getCurrent();
            assertFalse("Monitor assertions: " + ctx.buildFailureReport(),
                    ctx.hasAssertionFailures());
            CapturedApiCall call = ctx.getLastApiCall("/api/users");
            // ⭐ 若 MONITOR 成功读到响应（非竞态失败），则必须是真实 200 数据（非 mock）
            if (call != null) {
                assertEquals("Status code should be real 200", 200L, (long) call.statusCode());
                assertTrue("Response should be real data (not mocked)",
    ...
    public void n54_context_rules_cleanup() {
        ...
        try {
            ...
            CapturedApiCall beforeCall = ApiCaptureContext.getCurrent().getLastApiCall("/api/users");
            ...
            CapturedApiCall afterCall = ApiCaptureContext.getCurrent().getLastApiCall("/api/users");
    ...
    @Test
    public void n56_mock_always_shortcircuits_other_types_same_pattern() throws Exception {
        // ⭐ 同一 pattern 下，MOCK 是唯一、无条件的终结者：
        //   • 无论 MODIFY/DELAY/MONITOR 在 MOCK 之前还是之后注册，只要有 MOCK 就短路，直接返回 mock；
        //   • MONITOR 对 MODIFY/DELAY 是「不可覆盖的叠加基线」——非 MOCK 组合下监控必须一直生效。
        // 验证「先 modify 后 mock」「先 mock 后 modify」「先 delay 后 mock」三条同 pattern 顺序场景。

        // 场景 1：先 MODIFY（改请求头）后 MOCK（同 pattern）→ MOCK 短路，返回 mock，不真实请求
        RouteDsl modThenMock = RouteDsl.on(page)
                .api("/api/users/101")
                .modifyRequest()
                .setRequestHeader("X-Request-ID", "will-be-shortcircuited")
                .allowAllRequests()
                .done();
        modThenMock.start();
        RouteDsl mock1 = RouteDsl.on(page)
                .api("/api/users/101")
                .mock()
                .mockBody("{\"id\":101,\"name\":\"MOCK-101\",\"source\":\"mock\"}")
                .mockStatus(200)
                .allowAllRequests()
                .done();
        mock1.start();

        // 场景 2：先 MOCK 后 MODIFY（同 pattern）→ MOCK 保持短路
        RouteDsl mock2 = RouteDsl.on(page)
                .api("/api/users/102")
                .mock()
                .mockBody("{\"id\":102,\"name\":\"MOCK-102\",\"source\":\"mock\"}")
                .mockStatus(200)
                .allowAllRequests()
                .done();
        mock2.start();
        RouteDsl modAfterMock = RouteDsl.on(page)
                .api("/api/users/102")
                .modifyRequest()
                .setRequestHeader("X-Request-ID", "ignored-because-mock")
                .allowAllRequests()
                .done();
        modAfterMock.start();

        // 场景 3：先 DELAY（1s）后 MOCK（同 pattern）→ MOCK 短路，立即返回（无 1s 延迟）
        RouteDsl delayThenMock = RouteDsl.on(page)
                .api("/api/users/103")
                .delay(1)
                .allowAllRequests()
                .done();
        delayThenMock.start();
        RouteDsl mock3 = RouteDsl.on(page)
                .api("/api/users/103")
                .mock()
                .mockBody("{\"id\":103,\"name\":\"MOCK-103\",\"source\":\"mock\"}")
                .mockStatus(200)
                .allowAllRequests()
                .done();
        mock3.start();
        ...
        try {
            // 触发三个请求
            navigateToApi("/api/users/101");
            navigateToApi("/api/users/102");
            navigateToApi("/api/users/103");
            ctx.awaitCompletion(10_000);

            // 断言：三条全部返回 mock（MOCK 无条件短路其它类型）
            // ⭐ DELAY 短路验证：若 DELAY(1s) 生效，navigateToApi 会阻塞到真实响应，
            //   返回的是真实数据而非 mock。这里拿到 mock 内容即证明 DELAY 已被 MOCK 短路。
            CapturedApiCall call101 = ctx.getLastApiCall("/api/users/101");
            assertNotNull("101 should be captured", call101);
            assertEquals("101 must be MOCK (modify short-circuited by mock)",
                    "MOCK-101", call101.json("$.name"));

            CapturedApiCall call102 = ctx.getLastApiCall("/api/users/102");
            assertNotNull("102 should be captured", call102);
            assertEquals("102 must be MOCK (mock keeps short-circuit after modify)",
                    "MOCK-102", call102.json("$.name"));

            CapturedApiCall call103 = ctx.getLastApiCall("/api/users/103");
            assertNotNull("103 should be captured", call103);
            assertEquals("103 must be MOCK (delay short-circuited by mock)",
                    "MOCK-103", call103.json("$.name"));
        } finally {
            modThenMock.clear();
            mock1.clear();
            mock2.clear();
