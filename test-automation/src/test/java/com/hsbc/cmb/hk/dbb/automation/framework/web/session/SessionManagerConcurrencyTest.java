...
public class SessionManagerConcurrencyTest {
    ...
    public void perThreadFeatureSessionIsolation() throws Exception {
        ...
        try {
            ...
            assertTrue("主线程应标记为已恢复", SessionManager.isFeatureSessionRestored("key1"));
            assertTrue("主线程 isAny 应为 true", SessionManager.isAnyFeatureSessionRestored());
            assertEquals("主线程应读到 homeUrl", "https://home.example.com", SessionManager.getFeatureHomeUrl());
            ...
            assertFalse("其他线程不应看到主线程的恢复标记", otherRestored.get(5, TimeUnit.SECONDS));
            assertFalse("其他线程不应看到主线程的 isAny 标记", otherAny.get(5, TimeUnit.SECONDS));
            assertNull("其他线程不应看到主线程的 homeUrl", otherHome.get(5, TimeUnit.SECONDS));
        } finally {
