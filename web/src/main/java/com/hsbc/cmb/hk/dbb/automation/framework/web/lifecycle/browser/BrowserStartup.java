...
/**
 * Browser 启动编排契约（WEB-P1-6 Phase 1 接口提取）。
 *
 * <p>默认实现 {@link BrowserStartupImpl}（WEB-P1-1 拆分成果的逻辑承载体，已实例化为单例）。
 * 该接口把「安装校验 / configId 生成 / Playwright 与 Browser 初始化 / 启动参数装配 /
 * 策略解析 / 连接建立」这组启动职责收敛为可替换契约，
 * 供 {@code PlaywrightRuntime} 实例门面与测试替身经此接口替换实现。</p>
 *
 * @see BrowserStartupImpl
 */
public interface BrowserStartup {

    boolean ensureBrowserInstalledForType();

    String generateConfigId();

    void initializePlaywright(String configId);

    Playwright.CreateOptions getCreateOptions();

    void initializeBrowser(String configId);

    void configureBrowserLaunchOptions(BrowserType.LaunchOptions launchOptions);

    BrowserStrategy resolveStrategy();

    Browser setupBrowser(Playwright playwright, String browserType, BrowserType.LaunchOptions launchOptions);
