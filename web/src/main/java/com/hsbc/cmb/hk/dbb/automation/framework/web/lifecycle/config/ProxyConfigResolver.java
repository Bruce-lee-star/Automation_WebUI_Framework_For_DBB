...
public final class ProxyConfigResolver {
    ...
    private static String getHttpsProxyUrlInternal() {
        String proxy = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTPS));
        ...
        if (proxy == null) {
            proxy = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTP));
        ...
        if (usingHttpFallback) {
            user = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTP_USERNAME));
            pass = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTP_PASSWORD));
        } else {
            user = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTPS_USERNAME));
            pass = nonBlank(FrameworkConfigManager.getString(WebFrameworkConfig.PLAYWRIGHT_PROXY_HTTPS_PASSWORD));
        }
        return buildUrl(proxy, user, pass);
    }
