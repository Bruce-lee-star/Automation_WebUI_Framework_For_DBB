 ...
 * 重入守卫（takingScreenshot）随本类 per-thread 持有，避免污染监听器的失败守卫状态。</p>
 *
...
final class FailureScreenshotHandler {
    ...
    private static final ThreadLocal<Boolean> TAKING_SCREENSHOT = ThreadLocal.withInitial(() -> false);
    ...
    /** scenario 结束时清理 per-thread 重入标记。 */
    static void clearThreadState() {
        TAKING_SCREENSHOT.remove();
    }
}
