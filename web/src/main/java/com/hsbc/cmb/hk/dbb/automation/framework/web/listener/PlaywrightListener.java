...
public class PlaywrightListener implements StepListener {
    ...
    public void testSkipped() {
        skippedTests.incrementAndGet();
        VerboseLogging.logInfoIfVerbose(logger, "Test skipped: {}", currentTestName.get());
        ...
        try {
            ...
            if (screenshotPath != null) {
                ...
                if (pngFile.exists()) {
                    ...
                    VerboseLogging.logDebugIfVerbose(logger, "Screenshot captured: {} -> {}", screenshotName, pngFile.getName());
    ...
    private void checkAndMarkApiAssertionFailures(TestOutcome result) {
        ...
        // 不在此处 reset()，由 cleanupThreadLocals() 统一调用 RouteLifecycleRegistry.get().resetCaptureCurrent()
    }
    ...
    private void checkAndFailOnApiAssertions() {
        ...
        //   StepInterceptor → Cucumber → JUnit4
        // 传播，使 IDE runner 正确标红。
        // 仅调用 StepEventBus.testFailed() 只影响 Serenity 报告模型，无法改变 JUnit4 判定，
        // 因此必须在此处抛出异常（Cucumber @After hook 在本框架的 Serenity 集成下不会被触发）。
        // apiFailureAlreadyHandled 已置位，StepEventBus.testFailed() 触发的回调不会重入此方法，无递归风险。
