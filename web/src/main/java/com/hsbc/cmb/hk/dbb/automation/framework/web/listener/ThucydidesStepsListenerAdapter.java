...
public class ThucydidesStepsListenerAdapter implements StepListener, FrameworkListener {
    ...
    public ThucydidesStepsListenerAdapter() {
        ...
        synchronized (lock) {
            if (!instanceCreated) {
                // 首次创建：记录INFO日志
                VerboseLogging.logInfoIfVerbose(logger, "🏗️ Created ThucydidesStepsListenerAdapter instance #{} (initializing singleton)", instanceId);
                VerboseLogging.logDebugIfVerbose(logger, "🔧 Initializing Thucydides Steps Listener Adapter (first time)");
                instance = this;
                ...
                try {
                    ...
                    VerboseLogging.logDebugIfVerbose(logger, " Created singleton PlaywrightListener");
                ...
                try {
                    ...
                    VerboseLogging.logDebugIfVerbose(logger, " Added AxeCoreListener for automatic accessibility testing");
                } catch (Exception e) {
...
/**
    ...
    public void addDelegateListener(StepListener listener) {
        if (listener != null && !delegateListeners.contains(listener)) {
            ...
            VerboseLogging.logInfoIfVerbose(
    ...
    public void removeDelegateListener(StepListener listener) {
        if (delegateListeners.remove(listener)) {
            VerboseLogging.logInfoIfVerbose(logger, "🗑️ Removed delegate listener: {} (total: {})", listener.getClass().getSimpleName(), delegateListeners.size());
        }
    }
    ...
    public void registerWithStepEventBus() {
        try {
            ...
            VerboseLogging.logInfoIfVerbose(logger, " Thucydides Steps Listener Adapter registered with StepEventBus");
        } catch (Exception e) {
    ...
    public void unregisterFromStepEventBus() {
        try {
            ...
            VerboseLogging.logInfoIfVerbose(logger, " Thucydides Steps Listener Adapter unregistered from StepEventBus");
        } catch (Exception e) {
    ...
    public void testStarted(String testName) {
        ...
        if (logger.isDebugEnabled() && instanceId == 1) {
            VerboseLogging.logDebugIfVerbose(logger, "🚀 Test started: {}", testName);
        }
    ...
    private void testFinishedInternal() {
        VerboseLogging.logDebugIfVerbose(logger, "🏁 Thucydides Adapter: Test finished");
    ...
    public void testSkipped() {
        VerboseLogging.logDebugIfVerbose(logger, "⏭️ Thucydides Adapter: Test skipped");
    ...
    public void stepFinished() {
        // 使用 VerboseLogging 控制日志输出
        VerboseLogging.logDebugIfVerbose(logger, " Step finished (delegates: {})", delegateListeners.size());
    ...
    public void stepIgnored() {
        VerboseLogging.logDebugIfVerbose(logger, "⏭️ Thucydides Adapter: Step ignored");
    ...
    public void clearDelegateListeners() {
        ...
        VerboseLogging.logInfoIfVerbose(logger, "🧹 Cleared all delegate listeners");
    }
...
@Override
    ...
    public void testSuiteFinished() {
        ...
        //    这里在 suite 结束时清零，使去重范围收敛到单个 suite 之内。
        lastTestSuiteName = null;
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void testStarted(String testName, String testMethod) {
        VerboseLogging.logDebugIfVerbose(logger, "Test started: {} with method: {}", testName, testMethod);
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void testStarted(String testName, String testMethod, ZonedDateTime startTime) {
        VerboseLogging.logDebugIfVerbose(logger, "Test started: {} with method: {} at: {}", testName, testMethod, startTime);
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void testFinished(TestOutcome result) {
        ...
        // 只在首次实例记录INFO日志
        if (instanceId == 1) {
            VerboseLogging.logDebugIfVerbose(logger, "Test finished with result: {}", result);
        }
    ...
    public void testFinished(TestOutcome result, boolean isInDataDrivenTest, ZonedDateTime finishTime) {
        ...
        // 只在首次实例记录INFO日志
        if (instanceId == 1) {
            VerboseLogging.logDebugIfVerbose(logger, "Test finished with result: {}, isDataDriven: {}, finishTime: {}", result, isInDataDrivenTest, finishTime);
        }
    ...
    public void testRetried() {
        VerboseLogging.logDebugIfVerbose(logger, "Test retried");
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void skippedStepStarted(ExecutedStepDescription step) {
        VerboseLogging.logDebugIfVerbose(logger, "Skipped step started: {}", step);
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void stepFailed(StepFailure failure, List<ScreenshotAndHtmlSource> screenshots, boolean takeScreenshotOnFailure, ZonedDateTime timestamp) {
        VerboseLogging.logDebugIfVerbose(logger, "Step failed with {} screenshots", screenshots != null ? screenshots.size() : 0);
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void stepPending() {
        VerboseLogging.logDebugIfVerbose(logger, "Step pending");
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void stepPending(String description) {
        VerboseLogging.logDebugIfVerbose(logger, "Step pending: {}", description);
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void stepFinished(List<ScreenshotAndHtmlSource> screenshots) {
        ...
        if (logger.isDebugEnabled() && (instanceId == 1 || (screenshots != null && screenshots.size() > 0))) {
            VerboseLogging.logDebugIfVerbose(logger, "Step finished with {} screenshots", screenshots != null ? screenshots.size() : 0);
        }
        for (StepListener listener : delegateListeners) {
            try {
    ...
    public void stepFinished(List<ScreenshotAndHtmlSource> screenshots, ZonedDateTime timestamp) {
        // 减少日志噪音：只在首次实例或有截图时记录
        if (logger.isDebugEnabled() && (instanceId == 1 || (screenshots != null && screenshots.size() > 0))) {
            VerboseLogging.logDebugIfVerbose(logger, "Step finished with {} screenshots at {}",
    ...
    public void testIgnored() {
        VerboseLogging.logDebugIfVerbose(logger, "Test ignored");
        for (StepListener listener : delegateListeners) {
            try {
